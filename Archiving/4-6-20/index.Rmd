...
The forecast of cumulative laboratory-confirmed cases using the current model simulations for Michigan is:
```{r forecasttable}
forecasttable = c("Date", "Uncertainty lower bound", "Best-fit", "Median", "Uncertainty upper bound")
forecasttable = rbind(forecasttable,
                      c(paste(format(lastdate+7, "%B %e, %Y"),'(1 week)')),
                      format(round_any(ymin[(lastdate + 7) - startdate + 1],10), scientific=F),
                      format(round_any(yobscumcarefun(xestlh,paramestslh)[(lastdate + 7) - startdate + 1],10), scientific=F),
                      format(round_any(ymax[(lastdate + 7) - startdate + 1],10), scientific=F))
forecasttable = rbind(forecasttable,
                      c(paste(format(lastdate+21, "%B %e, %Y"),'(1 week)')),
                      format(round_any(ymin[(lastdate + 21) - startdate + 1],10), scientific=F),
                      format(round_any(yobscumcarefun(xestlh,paramestslh)[(lastdate + 21) - startdate + 1],10), scientific=F),
                      format(round_any(ymax[(lastdate + 21) - startdate + 1],10), scientific=F))
