...
- The current best fit across all model simulations for Michigan gives a forecast of:
    - `r format(Sys.Date()+7, "%B %e, %Y")` (1 week): roughly `r format(round_any(yobscumcarefun(xestlh,paramestslh)[(Sys.Date() + 7) - startdate + 1],10), scientific=F)` cumulative laboratory-confirmed cases
    - `r format(Sys.Date()+21, "%B %e, %Y")` (3 weeks): roughly `r format(round_any(yobscumcarefun(xestlh,paramestslh)[(Sys.Date() + 21) - startdate + 1],100),scientific=F)` cumulative laboratory-confirmed cases
- The current best fit across model simulations projects:
    - `r format(Sys.Date()+7, "%B %e, %Y")` (1 week): roughly `r round_any(ycurrpatfun(xestlh,paramestslh)[(Sys.Date() + 7) - startdate + 1],100)` hospitalized patients (beds needed) with roughly `r round_any(ycurrICUfun(xestlh,paramestslh)[(Sys.Date() + 7) - startdate + 1],10)` in ICU 
    - `r format(Sys.Date()+21, "%B %e, %Y")` (3 weeks): roughly `r round_any(ycurrpatfun(xestlh,paramestslh)[(Sys.Date() + 21) - startdate + 1],100)` hospitalized patients (beds needed) with roughly `r round_any(ycurrICUfun(xestlh,paramestslh)[(Sys.Date() + 21) - startdate + 1],10)` in ICU 
...
forecastplot = ggplot(plotdata) +
  geom_line(aes(x=NumericDate,y=BestFit, text = paste("Date:",Date,"<br>Best fit:",round(BestFit)), group=1, 
                linetype = "Best fit")) +
  geom_line(aes(x=NumericDate,y=Median, text = paste("Date:",Date,"<br>Median:",round(BestFit)), group=1, 
                linetype = "Median")) +
